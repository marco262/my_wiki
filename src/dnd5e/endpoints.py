...
from src.dnd5e.utils import init_spells_and_magic_items, open_monster_sheet, load_spells, \
...
def load_wsgi_endpoints(app: Bottle):
    @app.get("/")
    def home():
        return md_page("5e Wiki", "dnd5e", build_toc=False)

    # Categories

    @app.get('/advancement/<name>')
    def advancement(name):
        return md_page(name, "dnd5e", "advancement")

    @app.get('/background/<name>')
    def background(name):
        return md_page(name, "dnd5e", "background")

    @app.get('/class/<name>')
    def dnd_class(name):
        return md_page(name, "dnd5e", "class")
    ...
    def general(name):
        return md_page(name, "dnd5e", "general")

    @app.get('/race/<name>')
    def race(name):
        return md_page(name, "dnd5e", "race")

    @app.get('/subclass/<name>')
    def subclass(name):
        return md_page(name, "dnd5e", "subclass")

    @app.get('/dm_toolbox/<name>')
    def subclass(name):
        return md_page(name, "dnd5e", "dm_toolbox")
